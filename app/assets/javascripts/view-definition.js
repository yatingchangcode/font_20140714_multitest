...
View.registerCanvas = (function(key){
  ...
  key = Commons.getCommonGenKey(key, ["A1+tv","A2+tv","A3+tv","B1+tv","B2_v1+tv","C1+tv","C2+tv","C3+tv"]);
...
View.setSubmitStyle = (function(key){
  ...
  return {
    ...
    "mix+tv":function(o){
      var cid = o.cid? o.cid : [o.user_id, 2, 2].join('_');
    ...
    "C5+tv":function(o){
      ...
      for (var i in o) {
        if (o[i]) {
          ...
          setTimeout( (function(uid){
            return function() {
              $("#no_correct_" + uid).text(Commons.tempcount[uid]).css('opacity', 1).css("color", "#feeb09");
            }
          })(i), 600 * total);
        }
      }
...
View.setUserOutStyle = (function(key){
  ...
  return {
    ...
    "C2+tv": function(o){
      ...
      $('#user_photo_' + o.user_id).parent().css("opacity", 0.3);
      $('#grid_' + o.user_id).css("opacity", 0.3);
      $('#sketchSecond_' + o.user_id).parent().css("opacity", 0.3);
      Commons.sketchSecondIns[o.user_id].clearBar();
    },
    "C5+tv": function(o){
      $('#out_' + o.user_id).show();
      $('#out_' + o.user_id).prev().css("opacity", 0.3);
      if(Settings.hasTimeCounter && !Settings.commonWriting)
        Commons.sketchSecondIns[o.user_id].clearBar();
      $('#user_photo_' + o.user_id).parent().css("opacity", 0.3);
      $('#sketchSecond_' + o.user_id).parent().css("opacity", 0.3);
...
View.setSendTextStyle = (function(key){
  ...
  // *** server: A1 A2 A3 B1 B2_v1 B2 no action
  key = Commons.getCommonGenKey(key, ["B3+console","B3+tv"]);
  key = Commons.getCommonGenKey(key, ["mix+console","mix+tv","C5+tv"]);
  return {
    // server: B3
    // tv: B3
    "B3+console":function(o){
